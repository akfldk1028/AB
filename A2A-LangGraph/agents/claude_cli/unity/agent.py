"""
Unity Agent - A2A Protocol Implementation with Claude CLI
"""
import os
import sys
import asyncio
...
class UnityCLIAgent:
    """Unity Agent that uses Claude CLI subprocess for responses"""
    
    SYSTEM_INSTRUCTION: Final[str] = (
        "You are a Unity Game Development expert specializing in Unity Engine, C#, "
        "game mechanics, graphics, and optimization. You help with game systems, "
        "shaders, multiplayer, VR/AR, and performance optimization."
    )
    ...
    async def invoke_async(self, query: Query, session_id: SessionId) -> AgentResponse:
        ...
        """
        print(f"[Unity Agent] Received query: {query[:100]}...")
        result = await self.invoke_claude_cli(query, session_id)
        print(f"[Unity Agent] Response status: task_complete={result.get('is_task_complete')}")
        return result
    ...
    async def stream(self, query: Query, session_id: SessionId) -> AsyncIterable[AgentResponse]:
        ...
        yield {
            "is_task_complete": False,
            "require_user_input": False,
            "content": "Processing Unity development request with Claude CLI..."
        }
...
if __name__ == "__main__":
    async def test() -> None:
        ...
        response = await agent.invoke_async(
            "Create a simple Unity character controller script",
            "test_session_123"
        )
