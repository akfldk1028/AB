"""
Vision Agent - A2A Protocol Implementation with Claude CLI
"""
import os
import sys
import asyncio
...
class VisionCLIAgent:
    """Vision Agent that uses Claude CLI subprocess for responses"""
    
    SYSTEM_INSTRUCTION: Final[str] = (
        "You are a Vision Language Model & LLM Integration specialist for Android XR applications. "
        "You handle AI vision processing, multimodal understanding, and intelligent analysis of visual "
        "data from XR environments. Focus on VLM integration, real-time processing, and XR optimization."
    )
    ...
    async def invoke_claude_cli(self, query: Query, session_id: SessionId) -> AgentResponse:
        ...
        try:
            ...
            print(f"[Vision Agent] Claude CLI Response: {len(response_text)} chars")
            if len(response_text) > 200:
                print(f"[Vision Agent] Preview: {response_text[:200]}...")
            else:
                print(f"[Vision Agent] Full Response: {response_text}")
    ...
    async def invoke_async(self, query: Query, session_id: SessionId) -> AgentResponse:
        ...
        """
        print(f"[Vision Agent] Received query: {query[:100]}...")
        result = await self.invoke_claude_cli(query, session_id)
        print(f"[Vision Agent] Response status: task_complete={result.get('is_task_complete')}")
        return result
    ...
    async def stream(self, query: Query, session_id: SessionId) -> AsyncIterable[AgentResponse]:
        ...
        yield {
            "is_task_complete": False,
            "require_user_input": False,
            "content": "Processing vision analysis request with Claude CLI..."
        }
...
if __name__ == "__main__":
    async def test() -> None:
        ...
        response = await agent.invoke_async(
            "Analyze this image using GPT-4V for XR scene understanding",
            "test_session_123"
        )
