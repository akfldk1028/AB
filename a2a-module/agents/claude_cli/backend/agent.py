...
class BackendCLIAgent:
    ...
    def enhance_query_with_mcp(self, query: Query) -> str:
        ...
        try:
            ...
            library_keywords = ["express", "fastapi", "django", "flask", "node", "python", "react", "vue", "angular"]
            found_library = None
            ...
            if len(query.split()) > 10 or "design" in query.lower() or "architecture" in query.lower():
                try:
                    thinking_result = self.mcp_client.sequential_thinking(
                        f"Analyze this backend development request: {query}"
                    )
            ...
            print(f"[Backend Agent] MCP enhancement failed: {str(e)}")
