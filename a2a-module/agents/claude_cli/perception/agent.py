"""
Perception Agent - A2A Protocol Implementation with Claude CLI
"""
import os
import sys
import asyncio
...
class PerceptionCLIAgent:
    """Perception Agent that uses Claude CLI subprocess for responses"""
    
    SYSTEM_INSTRUCTION: Final[str] = (
        "You are a Camera Perception & ROI Processing specialist for Android XR applications. "
        "You handle camera frame acquisition, region-of-interest processing, and real-time "
        "image preprocessing for XR experiences. Focus on Camera2, CameraX, OpenCV, and XR integration."
    )
    ...
    async def invoke_async(self, query: Query, session_id: SessionId) -> AgentResponse:
        ...
        """
        print(f"[Perception Agent] Received query: {query[:100]}...")
        result = await self.invoke_claude_cli(query, session_id)
        print(f"[Perception Agent] Response status: task_complete={result.get('is_task_complete')}")
        return result
